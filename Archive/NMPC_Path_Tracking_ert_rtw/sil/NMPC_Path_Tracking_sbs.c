/*
 * NMPC_Path_Tracking_sbs.c
 *
 * Automatically generated s-function with I/O interface for:
 * Component: NMPC_Path_Tracking
 * Component Simulink Path: NMPC_Path_Tracking
 * Simulation Mode: SIL
 *
 */

#define S_FUNCTION_NAME                NMPC_Path_Tracking_sbs
...
static boolean_T startAndSetupApplication(SimStruct *S)
{
...

  {
    ...
    {
      mxArray * codeGenComponent = mxCreateString("vehicle_hostmodel_2022b_v12");
      mxArray *rhs[3];
      const char * simulinkBlockPath = ssGetPath(S);
      rhs[ 0 ] = mxCreateString(
        "@coder.connectivity.SimulinkInterface.getSILPILInterface");
...
{
  {
    ...
    if (IOBufferPtr != NULL) {
      void * pXILService = (void *) ssGetPWorkValue(S, 9);
      if (pXILService != NULL) {
        if (xilHostAppSvcGrowIOBuffer(pXILService, IOBufferPtr, 2481, 0)!=
            XILHOSTAPPSVC_SUCCESS) {
          return;
        }                              /* if */
