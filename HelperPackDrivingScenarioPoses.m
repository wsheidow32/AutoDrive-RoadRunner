classdef HelperPackDrivingScenarioPoses < matlab.System
    ...
    methods(Access = protected)
        ...
        function TargetPoses = stepImpl(obj, time, msgs)
            ...
            % The RoadRunner Scenario reader block returns world actor
            % information along with the other actors present in scenario. 
            % In order to get only the target actors as an output, we avoid 
            % packing the ego actor and the world actor information. 

            TargetPoses.NumActors = numActors-2;
            iTarget = 1;
            for i = 1:numActors 
                if actors(i).ActorID ~= obj.EgoActorID && actors(i).ActorID ~= 0
